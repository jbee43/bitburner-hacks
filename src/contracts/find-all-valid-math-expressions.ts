...
class Expression {
    ...
    constructor(public term1: Term, public operator?: Operator, public term2?: Term) {
        ...
        switch (operator) {
            case "+":
                this.value = this.getTermValue(term1) + this.getTermValue(term2);
                break;
            case "-":
                this.value = this.getTermValue(term1) - this.getTermValue(term2);
                break;
    ...
    multiply(term: Term) {
        this.operator = "*";
        this.term2 = term;
        this.value = this.getTermValue(this.term1) * this.getTermValue(this.term2);
    }

    toString() {
        return `${this.term1}${this.operator !== undefined ? this.operator : ""}${
            this.term2 !== undefined ? this.term2 : ""
        }`;
    }
}

export default function findAllValidMathExpressions(ns: NS, arg: string) {
        ...
        for (const leftTerm of left) {
            for (const operator of operators) {
                if (leftTerm.term2 === undefined) {
                    expressions.push(new Expression(leftTerm.term1, operator, rightTerm));
                } else if (operator === "*") {
                    expressions.push(
                        new Expression(
                            leftTerm.term1,
                    ...
                    expressions.push(new Expression(leftTerm, operator, rightTerm));
                }
            }
        }
    }
